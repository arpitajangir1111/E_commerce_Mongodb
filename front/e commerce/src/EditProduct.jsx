...
function EditProduct() {
  ...
  const getProduct = async () => {
    let result = await axios.get(`http://localhost:8000/product/edit/${params.id}`)
    result = result.data
    setName(result.name)
  ...
  const updatedProduct = async () => {
    if (update) {
      let result = await axios.put(`http://localhost:8000/product/edit/${params.id}`, {
        name: name,
        price: price,
        category: category,
        company: company,
    ...
    if(!update){
      let result = await axios.put(`http://localhost:8000/product/update/${params.id}`,{name,price,category,company,image}, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      }
